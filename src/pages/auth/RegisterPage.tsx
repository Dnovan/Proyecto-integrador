...
import { Mail, Lock, Eye, EyeOff, User, Phone, UserPlus, Check, ArrowRight, Shield, Sparkles, Building } from 'lucide-react';
import { useAuth } from '../../context/AuthContext';

export const RegisterPage: React.FC = () => {
    const navigate = useNavigate();
    const [searchParams] = useSearchParams();
    const isProviderRegistration = searchParams.get('role') === 'PROVEEDOR';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await register({
                ...
                role: isProviderRegistration ? 'PROVEEDOR' : 'CLIENTE',
            });
                ...
                <div className="w-full max-w-md relative z-10 py-8">
                    ...
                    <div className="mb-8">
                        {isProviderRegistration && (
                            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-amber-100 to-yellow-100 rounded-full mb-4">
                                <Building className="w-4 h-4 text-amber-600" />
                                <span className="text-amber-700 text-sm font-semibold">Registro de Proveedor</span>
                            </div>
                        )}
                        <h1 className="text-3xl lg:text-4xl font-bold text-gray-900 mb-2">
                            {isProviderRegistration ? 'Únete como Proveedor' : 'Crea tu cuenta'}
                        </h1>
                        <p className="text-gray-700">
                            {isProviderRegistration
                                ? 'Publica tu espacio y comienza a recibir reservaciones'
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <button
                            type="submit"
                            disabled={isLoading}
                            className={`w-full py-3.5 px-6 text-white font-bold rounded-xl flex items-center justify-center gap-2 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 group mt-6 ${isProviderRegistration ? 'bg-gradient-to-r from-amber-500 to-yellow-500 hover:shadow-amber-500/30' : 'bg-gradient-to-r from-[#D4AF37] to-[#B8941F] hover:shadow-[#D4AF37]/30'}`}
                        >
                            {isLoading ? (
                                <>
                                ...
                                <>
                                    {isProviderRegistration ? <Building className="w-5 h-5" /> : <UserPlus className="w-5 h-5" />}
                                    <span>{isProviderRegistration ? 'Registrarme como Proveedor' : 'Crear Cuenta'}</span>
