    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            try {
                ...
                setGuestCount(venueData.capacity);
            } catch (error) {
        ...
        <div className="min-h-screen bg-bg-primary pt-16">
            ...
            <section className="relative">
                        ...
                        >
                            ...
                            {idx === 3 && venue.images.length > 5 && (
                                <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                                    <span className="text-white font-semibold">+{venue.images.length - 5} más</span>
                                </div>
                            )}
                        </div>
                    ))}
                ...
                <div className="absolute bottom-4 right-4 flex gap-2">
                    <Button variant="secondary" size="sm" leftIcon={<Share2 className="w-4 h-4" />}>Compartir</Button>
                    <Button
            ...
            <section className="max-w-7xl mx-auto px-4 py-8">
                <div className="grid lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-8">
                        ...
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                {venue.status === 'FEATURED' && <Badge variant="neon">Destacado</Badge>}
                                <Badge>{api.categoryLabels[venue.category]}</Badge>
                            </div>
                            <h1 className="text-3xl md:text-4xl font-bold text-text-primary mb-2">{venue.name}</h1>
                            <div className="flex flex-wrap items-center gap-4 text-text-secondary">
                                <div className="flex items-center gap-1"><MapPin className="w-4 h-4" /><span>{venue.address}</span></div>
                                <div className="flex items-center gap-1"><Users className="w-4 h-4" /><span>Hasta {venue.capacity} personas</span></div>
                        ...
                        <Card variant="default">
                            <h2 className="text-xl font-semibold text-text-primary mb-4">Descripción del espacio</h2>
                                        ...
                                        <div className="space-y-3">
                                            {venue.services.map((service) => {
                                                ...
                                                return (
                                                    <div
                                                        key={service.id}
                                                        className={`flex items-center p-4 rounded-xl transition-all ${isSelected ? 'shadow-md' : 'bg-gray-50 hover:bg-gray-100 border border-gray-200'} ${service.isOptional ? 'cursor-pointer' : ''}`}
                                                        style={isSelected ? { backgroundColor: 'rgba(212, 175, 55, 0.1)', border: '2px solid rgba(212, 175, 55, 0.4)' } : {}}
                                                        onClick={() => service.isOptional && handleServiceToggle(service.id)}
                                                    >
                                                        {/* Checkbox cuadrado */}
                                                        <div className={`w-5 h-5 flex-shrink-0 rounded-md flex items-center justify-center border-2 mr-4 ${isSelected ? 'border-transparent' : 'border-gray-300 bg-white'}`} style={isSelected ? { backgroundColor: '#D4AF37' } : {}}>
                                                            {isSelected && <Check className="w-3 h-3 text-white" strokeWidth={3} />}
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <div className="flex items-center gap-2">
                                                                <p className="font-semibold text-gray-900">{service.name}</p>
                                                                {isIncluded && <span className="text-[10px] uppercase font-bold px-2 py-0.5 rounded-full bg-green-100 text-green-700">Incluido</span>}
                                                            </div>
                                                        </div>
                                                        <div className="flex-shrink-0 ml-4 text-right">
                                                            {isIncluded ? (
                                                                <span className="text-sm font-bold" style={{ color: '#D4AF37' }}>INCLUIDO</span>
                                                            ) : (
                                                                <span className="text-sm font-bold text-gray-700">+{formatPrice(service.price)}</span>
                                                            )}
                                                        </div>
                                                    </div>
                                                );
                        ...
                        {/* Calendario Grande */}
                        <div className="p-6 rounded-2xl border-2 bg-white" style={{ borderColor: 'rgba(212, 175, 55, 0.3)' }}>
                            <div className="flex items-center justify-between mb-6">
                                <div>
                                    <h3 className="text-xl font-bold text-gray-900">Disponibilidad</h3>
                                    <p className="text-gray-500 text-sm mt-1">Selecciona una fecha para tu evento</p>
                                </div>
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="font-bold text-gray-900 min-w-[180px] text-center capitalize">
                            ...
                            <div className="grid grid-cols-7 gap-2 text-center mb-4">
                                {['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'].map((d, i) => (
                                    <div key={i} className="text-gray-500 font-semibold py-2 text-sm">{d}</div>
                                ))}
                            </div>
                            <div className="grid grid-cols-7 gap-2 text-center">
                                {getDaysInMonth(currentMonth).map((day, idx) => (
                                    <div key={idx}>
                                        {day !== null ? (
                                            <button
                                                className={`w-full aspect-square rounded-xl text-sm font-semibold transition-all flex items-center justify-center ${isDateAvailable(day)
                                                ...
                                                style={selectedDate === `${currentMonth.getFullYear()}-${String(currentMonth.getMonth() + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}` ? { backgroundColor: '#D4AF37', boxShadow: '0 4px 12px rgba(212, 175, 55, 0.4)' } : {}}
                            ...
                            <div className="flex items-center justify-center gap-8 mt-6 pt-4 border-t border-gray-100">
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 rounded-md" style={{ backgroundColor: '#D4AF37' }} />
                                    <span className="text-gray-600 text-sm">Seleccionada</span>
                                </div>
                                <div className="flex items-center gap-2">
                        ...
                        {/* Métodos de pago */}
                        <Card variant="default">
                            <h2 className="text-xl font-semibold text-text-primary mb-4">Métodos de pago aceptados</h2>
                        ...
                        <Card variant="default">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-xl font-semibold text-text-primary">Reseñas ({venue.reviewCount})</h2>
                                                ...
                                                <div className="flex items-center justify-between mb-2">
                                                    <div>
                                                        ...
                                                        <p className="text-sm text-text-muted">
                                                            {new Date(review.createdAt).toLocaleDateString('es-MX', { month: 'long', year: 'numeric' })}
                                                        </p>
                                                    </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-24 p-5 rounded-2xl border-2 bg-white" style={{ borderColor: 'rgba(212, 175, 55, 0.3)', backgroundColor: 'rgba(212, 175, 55, 0.05)' }}>
                            ...
                            <Button fullWidth size="lg" disabled={!selectedDate} className="mb-3" onClick={() => { if (!isAuthenticated) { setShowLoginModal(true); } }}>
            ...
            {isLightboxOpen && (
                <div className="fixed inset-0 z-50 bg-black/95 flex items-center justify-center">
                    <button className="absolute top-4 right-4 p-2 text-white hover:text-neon transition-colors" onClick={() => setIsLightboxOpen(false)}>
                        <X className="w-8 h-8" />
                    </button>
                    <button className="absolute left-4 p-2 text-white hover:text-neon transition-colors" onClick={prevImage}>
                        <ChevronLeft className="w-8 h-8" />
                    </button>
                    <img src={venue.images[currentImageIndex]} alt={`${venue.name} ${currentImageIndex + 1}`} className="max-h-[90vh] max-w-[90vw] object-contain" />
                    <button className="absolute right-4 p-2 text-white hover:text-neon transition-colors" onClick={nextImage}>
                        <ChevronRight className="w-8 h-8" />
                    </button>
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                        {venue.images.map((_, idx) => (
                            <button key={idx} className={`w-2 h-2 rounded-full transition-all ${idx === currentImageIndex ? 'bg-neon w-4' : 'bg-white/50'}`} onClick={() => setCurrentImageIndex(idx)} />
                        ))}
                    </div>
                </div>
            )}
            ...
            <LoginRequiredModal isOpen={showLoginModal} onClose={() => setShowLoginModal(false)} action="solicitar una reservación" />
        </div>
    );
};
