...
class InfiniteGridMenu {
    ...
    constructor(canvas: HTMLCanvasElement, items: any[], onActiveItemChange: any, onMovementChange: any, onInit: any = null, scale = 1.0) {
        this.canvas = canvas;
        this.items = items || [];
    ...
    #init(onInit: any) {
        ...
        this.instancePositions = this.icoGeo.vertices.map((v: any) => v.position);
                ...
                item =>
                    new Promise((resolve) => {
                        ...
                        img.onerror = () => {
                            ...
                            const ctx = canvas.getContext('2d');
                            if (ctx) {
                                ctx.fillStyle = '#D4AF37'; // Gold fallback
    ...
    #onControlUpdate(deltaTime: any) {
        ...
        if (!this.control.isPointerDown) {
            // @ts-ignore
            const nearestVertexIndex: number = this.#findNearestVertexIndex();
    ...
    useEffect(() => {
        ...
        if (canvas) {
            sketch = new InfiniteGridMenu(
                ...
                () => { }, // No-op for active item change
                () => { }, // No-op for movement change
